...
#' @seealso \code{\link{}}
#' 
analyse.selection = function(res_model, data_version, variable, person, nom)
{
  
  data=unique(data_version[,c("year","location","germplasm","group","type")])
 ...
 compare = function(x){
  ...
  if (length(mel) > 0 & length(mel) < nrow(Data)) {p = p + annotate("text",label = rep("*",length(mel)),x=mel,y=0, size=10)}
